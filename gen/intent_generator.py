...
def loadRequests():
	...
	requests = []
	for line in lines:
		wrap_line = "ENDPOINT"+","+line.strip()+","+"ENDPOINT"
		requests.append(wrap_line)
	return requests

def requestToVar(req):
	global dic,dupList,pairList
	vnflist = reqToNum(req,dic)
	vnfarcs = reqToArc(req,dupList)
	p_to_s,p_to_d = generateProximity(req,pairList)
	p_to_s = [str(x) for x in p_to_s]
	p_to_d = [str(x) for x in p_to_d]

	return vnflist,vnfarcs,p_to_s,p_to_d 
